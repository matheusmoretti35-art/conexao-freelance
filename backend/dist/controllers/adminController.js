...
exports.alterarStatusManualmente = exports.obterDashboardAdmin = void 0;
const db_1 = require("../db");
/**
 * 1. OBTÊR MÉTRICAS E DADOS DO DASHBOARD ADMINISTRATIVO
 * Endpoint: GET /api/admin/dashboard
 */
const obterDashboardAdmin = async (req, res) => {
    try {
      ...
      FROM prestadores;
    `;
        const metricasRes = await db_1.pool.query(metricasQuery);
      ...
      LIMIT 100;
    `;
        const prestadoresRes = await db_1.pool.query(prestadoresQuery);
      ...
      LIMIT 20;
    `;
        const pagamentosRes = await db_1.pool.query(pagamentosQuery);
...
const alterarStatusManualmente = async (req, res) => {
    try {
        ...
        const updateRes = await db_1.pool.query(`UPDATE prestadores SET status = $1, atualizado_em = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, nome_completo, status;`, [novo_status, id]);
...
exports.alterarStatusManualmente = alterarStatusManualmente;
