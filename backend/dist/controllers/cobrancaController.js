...
exports.verificarStatusPagamento = exports.receberWebhookPagamento = exports.gerarCheckout = void 0;
 ...
 */
const gerarCheckout = async (req, res) => {
    ...
    try {
        ...
        const assRes = await client.query(`INSERT INTO assinaturas (prestador_id, gateway_subscription_id, valor, metodo_pagamento, status, data_proxima_cobranca)
       VALUES ($1, $2, 29.90, $3, 'aguardando_pagamento', $4) RETURNING id;`, [prestador_id, `sub_${mockTxId}`, metodo_pagamento, dataProxima]);
        // 2. Registra Transação / Fatura
        const txRes = await client.query(`INSERT INTO transacoes (assinatura_id, prestador_id, gateway_transaction_id, valor, metodo_pagamento, status, pix_copia_cola)
       VALUES ($1, $2, $3, 29.90, $4, 'aguardando_pagamento', $5)
       RETURNING id, gateway_transaction_id, pix_copia_cola, status;`, [assRes.rows[0].id, prestador_id, mockTxId, metodo_pagamento, mockPixPayload]);
...
const receberWebhookPagamento = async (req, res) => {
    try {
        ...
        if (evento === 'PAYMENT_RECEIVED' || status === 'pago') {
            ...
            await db_1.pool.query(`UPDATE transacoes SET status = 'pago', data_pagamento = CURRENT_TIMESTAMP WHERE gateway_transaction_id = $1;`, [transaction_id]);
...
exports.receberWebhookPagamento = receberWebhookPagamento;
/**
...
const verificarStatusPagamento = async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('SELECT status, data_pagamento FROM transacoes WHERE gateway_transaction_id = $1', [tx]);
...
exports.verificarStatusPagamento = verificarStatusPagamento;
