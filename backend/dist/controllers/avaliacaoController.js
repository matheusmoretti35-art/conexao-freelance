...
exports.listarAvaliacoesPrestador = exports.publicarReplica = exports.criarAvaliacao = exports.registrarContato = void 0;
const db_1 = require("../db");
/**
 * 1. ROTA DE REGISTRO DE CONTATO (Chamada quando o cliente clica para abrir WhatsApp)
 * Endpoint: POST /api/contatos
 */
const registrarContato = async (req, res) => {
    try {
        ...
        await db_1.pool.query(`INSERT INTO contatos_clientes (prestador_id, cliente_whatsapp, cliente_nome) VALUES ($1, $2, $3);`, [prestador_id, cliente_whatsapp.trim(), cliente_nome ? cliente_nome.trim() : null]);
...
exports.registrarContato = registrarContato;
/**
...
const criarAvaliacao = async (req, res) => {
    try {
        ...
        const contatoCheck = await db_1.pool.query(`SELECT id FROM contatos_clientes WHERE prestador_id = $1 AND cliente_whatsapp = $2`, [prestador_id, cliente_whatsapp.trim()]);
        ...
        const insertRes = await db_1.pool.query(`INSERT INTO avaliacoes (prestador_id, cliente_nome, cliente_whatsapp, nota, comentario)
       VALUES ($1, $2, $3, $4, $5) RETURNING id, criado_em;`, [prestador_id, cliente_nome.trim(), cliente_whatsapp.trim(), nota, comentario.trim()]);
 ...
 */
const publicarReplica = async (req, res) => {
    try {
        ...
        const avCheck = await db_1.pool.query(`SELECT id FROM avaliacoes WHERE id = $1 AND prestador_id = $2`, [avaliacao_id, prestador_id]);
        ...
        const insertRes = await db_1.pool.query(`INSERT INTO replicas_prestadores (avaliacao_id, prestador_id, resposta) VALUES ($1, $2, $3) RETURNING id, criado_em;`, [avaliacao_id, prestador_id, resposta.trim()]);
...
const listarAvaliacoesPrestador = async (req, res) => {
    ...
    `;
        const result = await db_1.pool.query(queryText, [id]);
...
exports.listarAvaliacoesPrestador = listarAvaliacoesPrestador;
